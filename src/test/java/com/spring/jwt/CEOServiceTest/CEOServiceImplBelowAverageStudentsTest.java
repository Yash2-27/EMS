...
class CEOServiceImplBelowAverageStudentsTest {
    ...
    void setUp() {
        ...
        examResults = List.of(
                createExamResult(user1, 45), createExamResult(user1, 40), createExamResult(user1, 50), createExamResult(user1, 35),
                createExamResult(user2, 30), createExamResult(user2, 25), createExamResult(user2, 40), createExamResult(user2, 45),
                createExamResult(user3, 55), createExamResult(user3, 60), createExamResult(user3, 50), createExamResult(user3, 45)
        );
    }
    ...
    void testCalculateStudentPercentages() {
        ...
        assertEquals(42.5, percentages.get(1), 0.01);
        assertEquals(35.0, percentages.get(2), 0.01);
        assertEquals(52.5, percentages.get(3), 0.01);
    }

    @Test
    void testGetBelowAverageStudents() {
        ...
        List<DashboredDTO> belowAverageStudents = service.getBelowAverageStudents("11", "2025");

        assertEquals(2, belowAverageStudents.size());
        assertEquals("Pratham", belowAverageStudents.get(0).getName());
        assertEquals(42.5, belowAverageStudents.get(0).getPercentage(), 0.01);
        assertEquals("Amit", belowAverageStudents.get(1).getName());
        assertEquals(35.0, belowAverageStudents.get(1).getPercentage(), 0.01);
    }

    @Test
    void testGetBelowAverageStudents_noStudentsFound() {
        when(studentRepository.findByStudentClassAndBatch("11", "2025")).thenReturn(List.of());
        assertThrows(ResourceNotFoundException.class, () -> service.getBelowAverageStudents("11", "2025"));
    }

    @Test
    void testGetBelowAverageStudents_invalidInput() {
        assertThrows(IllegalArgumentException.class, () -> service.getBelowAverageStudents(null, "2025"));
        assertThrows(IllegalArgumentException.class, () -> service.getBelowAverageStudents("11", ""));
    }
